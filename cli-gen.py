...
def get_api_key() -> str:
    ...
    # check in the file
    current_directory = os.path.dirname(os.path.abspath(__file__))  # cli-gen.py location
    try:
        with open(os.path.join(current_directory, "open_ai_api_key.txt")) as api_key_file:
    ...
    print("Go to Go to https://platform.openai.com/account/api-keys to generate one for yourself")
    print("OpenAI offers a free $5 worth credits for everyone once.")
    print("Either paste the key here to store it next to cli-gen.py or save it as OPENAI_API_KEY yourself.")
    key = input("Key: ")
    ...
    print(f"Thanks, api key saved at {os.path.join(current_directory, 'open_ai_api_key.txt')}\n")
